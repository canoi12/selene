 ...
 ************************/
#if defined(OS_ANDROID)
static MODULE_FUNCTION(sdl, android_back_button) {
    SDL_AndroidBackButton();
    return 0;
}

static MODULE_FUNCTION(sdl, android_get_external_storage_path) {
    const char* path = SDL_AndroidGetExternalStoragePath();
    PUSH_STRING(path);
    return 1;
}

static MODULE_FUNCTION(sdl, android_get_internal_storage_path) {
...
const Uint8 *keys;
static MODULE_FUNCTION(sdl, check_key_state) {
  INIT_ARG();
  CHECK_INTEGER(key);
...
static MODULE_FUNCTION(sdl, set_time) { return 1; }
...
static MODULE_FUNCTION(sdl, enable_screen_saver) { SDL_EnableScreenSaver(); return 0; }
...
SELENE_API int luaopen_sdl(lua_State* L) {
    BEGIN_REG(reg)
        REG_FIELD(sdl, init),
        REG_FIELD(sdl, quit),
#if defined(OS_ANDROID)
        ...
        /* Android */
    #if defined(OS_ANDROID)
        REG_FIELD(sdl, android_back_button),
        REG_FIELD(sdl, android_get_external_storage_path),
        REG_FIELD(sdl, android_get_internal_storage_path),
    #endif
        /* AudioDevice */
