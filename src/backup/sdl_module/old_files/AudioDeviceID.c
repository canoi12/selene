...
static MODULE_FUNCTION(sdlAudioDeviceID, open) {
    INIT_ARG();
...
static MODULE_FUNCTION(sdlAudioDeviceID, get_count) {
    INIT_ARG();
    GET_BOOLEAN(is_capture);
...
static MODULE_FUNCTION(sdlAudioDeviceID, get_name) {
    INIT_ARG();
    CHECK_INTEGER(index);
    GET_BOOLEAN(is_capture);
...
BEGIN_META(sdlAudioDeviceID) {
    BEGIN_REG(reg)
        REG_FIELD(sdlAudioDeviceID, open),
        REG_FIELD(sdlAudioDeviceID, get_count),
        REG_FIELD(sdlAudioDeviceID, get_name),
    END_REG()
    ...
    END_REG()
    NEW_META(sdlAudioDeviceID, reg, index_reg);
    return 1;
}
