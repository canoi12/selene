...
static int get_virt_path(lua_State* L, const char* path) {
    const char* start = strstr(path, "exec://");
    int virt_path = -1;
    if (start) virt_path = r_exec_path;
    start = strstr(path, "root://");
    if (start) virt_path = r_root_path;
