...
static inline int l_renderer_Texture2D_destroy(lua_State* L) {
...
static inline int l_renderer_Texture2D_set_filter(lua_State* L) {
    CHECK_META(Texture2D);
...
int l_Texture2D_open_meta(lua_State* L) {
    ...
    const luaL_Reg reg[] = {
        REG_FIELD(renderer_Texture2D, create),
        REG_FIELD(renderer_Texture2D, destroy),
        REG_FIELD(renderer_Texture2D, set_filter),
        {NULL, NULL}
    };
